...
import url from 'url'
import { randomString } from '@nuxtjs/auth/lib/core/utilities'
...
describe('Logging In - Single Sign on', function () {
  ...
  context('Use redirectTo and a session cookie to login', function () {
    ...
    it('is 403 unauthorized without a session cookie', function () {
      ...
      // visit the dashboard
      cy.visit('/discussion/new?iri=http%3A%2F%2Fexample.org%2Ffoo', { failOnStatusCode: false })
      cy.get('title').should('contain', 'You are not allowed to see this')
    })
    ...
    it('can authenticate with cy.request', function () {
      ...
      cy.get('button').should('contain', 'Welcome to the Dashboard')
